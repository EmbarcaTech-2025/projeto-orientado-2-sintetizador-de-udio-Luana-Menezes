...
void init_pwm() {
    ...
    pwm_config_set_wrap(&config, ADC_MAX_VALUE);
...
void play_audio(){
  ...
  for(int i = 0; i < BUFFER_SIZE; i++) {
    pwm_set_gpio_level(BUZZER_PIN, audio_buffer[i]);
...
void display_audio_waveform(){
  ...
  for(int x = 0; x < ssd1306_width; x++) {
    ...
    if (bar_height > center_y) bar_height = center_y;
